...
def make_media_filename(meta, ext):
    sc = meta.get("shortcode") or meta.get("id") or "media"
...
def extract_tagged_usernames(node):
    if not node or not isinstance(node, dict):
        return []
    usernames = []
    for key in ("edge_media_to_tagged_user", "usertags", "tagged_users"):
        tag_block = node.get(key)
        if isinstance(tag_block, dict):
            edges = tag_block.get("edges") or tag_block.get("nodes") or []
            for item in edges:
                if not isinstance(item, dict):
                    continue
                tagged = item.get("node") or item
                if isinstance(tagged, dict):
                    username = (tagged.get("user") or {}).get("username") or tagged.get("username")
                    if username:
            ...
            for item in tag_block:
                if not isinstance(item, dict):
                    continue
                username = (item.get("user") or {}).get("username") or item.get("username")
                if username:
                    usernames.append(username)
...
def build_metadata(node, source="timeline"):
    ...
    tagged_users = extract_tagged_usernames(node)
    music_info = ((node.get("clips_metadata") or {}).get("music_info") or {})
    ...
    return {
        ...
        "owner_pk":                  owner.get("pk") or owner.get("id"),
        ...
        "collab_with":               ", ".join(tagged_users),
        "collab_count":              len(tagged_users),
        "profile_pk":                owner.get("pk") or owner.get("id"),
        "profile_username":          owner.get("username"),
        "profile_fullname":          owner.get("full_name"),
...
# ==================== DOWNLOADING ====================
def media_item_is_video(media_item):
    if not isinstance(media_item, dict):
        return False
    if media_item.get("type") == "video":
        return True
    meta = media_item.get("metadata") or {}
    if meta.get("video_url") or meta.get("video_versions"):
        return True
    return False
...
def download_media_item(media_item, out_dir, label, idx, total):
    ...
    is_video = media_item_is_video(media_item)
    ext = ".mp4" if is_video else ".jpg"
    fname = make_media_filename(meta, ext)
    meta["download_filename"] = fname
...
def save_metadata(username, media_items, profile_info, base_dir):
    ...
    def _rows(items):
        ...
        for item in items:
            ...
            rows.append({
                ...
                "tagged_users":               json.dumps(m.get("tagged_users") or [], ensure_ascii=False),
                "collab_with":                m.get("collab_with"),
                "collab_count":               m.get("collab_count"),
                "likes":                      m.get("likes"),
                ...
                "profile_pk":                 m.get("profile_pk"),
                "profile_username":           m.get("profile_username"),
                "profile_fullname":           m.get("profile_fullname"),
                "analysis_date":              m.get("analysis_date"),
                "scraped_at":                 m.get("scraped_at"),
                "scraped_date":               m.get("scraped_date"),
    ...
    combined_csv = os.path.join(base_dir, f"{username}_profile_media_metadata.csv")

    if rows:
        pd.DataFrame(rows).to_csv(merged_csv, index=False, encoding="utf-8-sig")

    profile_flat = flatten_json(profile_info or {})
    profile_row_prefix = {f"profile_{k}": v for k, v in profile_flat.items()}
    combined_rows = []
    for row in rows:
        merged = dict(profile_row_prefix)
        merged.update(row)
        combined_rows.append(merged)

    if combined_rows:
        pd.DataFrame(combined_rows).to_csv(combined_csv, index=False, encoding="utf-8-sig")
    elif profile_row_prefix:
        pd.DataFrame([profile_row_prefix]).to_csv(combined_csv, index=False, encoding="utf-8-sig")
...
def save_global_video_metadata(base_dir):
    if not all_video_metadata:
        return

    videos = []
    for item in all_video_metadata:
        if not media_item_is_video(item):
            continue
        meta = dict(item["metadata"])
        meta["download_url"] = item.get("url")
        if "download_filename" not in meta:
            meta["download_filename"] = make_media_filename(meta, ".mp4")
        videos.append(meta)
    ...
    output_path = os.path.join(base_dir, "all_videos_metadata.json")
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    log(f"Global video metadata saved ({len(videos)} videos) → {output_path}", "SUCCESS")
...
def process_profile(profile_url, driver, stats):
    ...
    video_items = [item for item in media_items if media_item_is_video(item)]
    if video_items:
        with metadata_lock:
            for item in video_items:
                item["metadata"]["download_filename"] = make_media_filename(item["metadata"], ".mp4")
                all_video_metadata.append(item)
...
def main():
    ...
    save_global_video_metadata("videos")
