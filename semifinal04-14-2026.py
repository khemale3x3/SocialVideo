...
def scrape_section(driver, url, section_label, post_list: list, seen_ids: set, seen_req_ids: set):
    ...
    Returns a tuple of (collected_nodes, profile_info).
    """
...
# ==================== DOWNLOADING ====================
def download_video(video_dict, out_dir, label, idx, total):
    url      = video_dict["url"]
    meta     = video_dict["metadata"]
    sc       = meta.get("shortcode") or meta.get("id") or f"vid_{idx}"
    ts       = meta.get("taken_at_fmt", "")[:10].replace("-", "") if meta.get("taken_at_fmt") else "unknown"
    cap_snip = clean_filename(meta.get("caption", "")[:35])
    fname    = f"{ts}_{sc}{'_' + cap_snip if cap_snip else ''}.mp4"
